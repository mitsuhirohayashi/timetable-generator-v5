...
def main():
        ...
        if exchange_assignment.subject != parent_assignment.subject:
            ...
            try:
                ...
                if not exchange_teacher:
                    logger.warning(f"{exchange_class}に{parent_assignment.subject.name}の教師が割り当てられていません")
                    # 親学級と同じ教師を使用
                ...
                if schedule.assign(time_slot, new_assignment):
                    logger.info(f"✓ {exchange_class}を{parent_assignment.subject.name}に更新しました")
                    fixed_count += 1
                else:
                    logger.error(f"✗ {exchange_class}の更新に失敗しました")
    ...
    if fixed_count > 0:
        ...
        schedule_repo.save(schedule, path_manager.output_dir / "output.csv")
        ...
        for day in ['月', '火', '水']:
            ...
            for exchange_class, parent_class in exchange_pairs.items():
                ...
                if exchange_assignment and parent_assignment:
                    ...
                    logger.info(f"  {exchange_class}: {exchange_subject} / {parent_class}: {parent_subject} {status}")
    else:
