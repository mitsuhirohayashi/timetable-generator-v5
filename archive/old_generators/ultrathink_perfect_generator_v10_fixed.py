#!/usr/bin/env python3
"""
Ultrathink Perfect Generator V10 Fixed - Teacher Availability Issue Fixed
Fixes the teacher availability initialization to properly detect existing assignments
"""
...
class UltrathinkPerfectGeneratorV10Fixed:
    """最初から完璧な時間割を生成する革新的ジェネレーター（V10 Fixed）"""
    ...
    def __init__(self):
        self.stats = {
            ...
            'existing_teacher_assignments': 0  # 新しい統計項目
        }
    ...
    def generate(self, school: School, constraints: List[Constraint],
        ...
        """完璧な時間割を生成"""
        logger.info("=== Ultrathink Perfect Generator V10 Fixed 開始 ===")
        ...
        self.teacher_availability = self._initialize_teacher_availability_fixed(school, schedule)
    ...
    def _initialize_teacher_availability_fixed(self, school: School, schedule: Schedule) -> Dict[str, Set[Tuple[str, int]]]:
        """教師の利用可能時間を初期化（修正版）"""
        availability = {}
        
        # 全教師の全時間を利用可能として初期化
        all_teachers = school.get_all_teachers()
        logger.info(f"全教師数: {len(all_teachers)}")
        ...
        for time_slot, assignment in schedule.get_all_assignments():
            ...
            if assignment.subject.name in self.fixed_subjects:
                logger.debug(f"固定科目をスキップ: {assignment.subject.name} @ {time_slot}")
                continue
    ...
    def _print_statistics(self, schedule: Schedule, school: School) -> None:
        ...
        logger.info("=== 統計情報 ===")
        logger.info(f"初期割り当て数: {self.stats['initial_assignments']}")
        logger.info(f"既存教師配置数: {self.stats['existing_teacher_assignments']}")
        ...
        logger.info(f"最終割り当て数: {total_assignments}")
        
        # 空きスロット数
        total_slots = len(school.get_all_classes()) * 5 * 6  # クラス数 × 曜日 × 時限
