#!/usr/bin/env python3
"""
Ultrathink Perfect Generator V8（教師重複修正版）
V7の教師重複問題を修正

V7からの改善点：
1. 既存配置の教師をbusy状態として正しく初期化
2. 配置時の教師利用可能性チェックを強化
3. デバッグログの追加
"""
...
class UltrathinkPerfectGeneratorV8:
    """最初から完璧な時間割を生成する革新的ジェネレーター（V8）"""
    ...
    def __init__(self):
        self.stats = {
            ...
            'teacher_conflicts_avoided': 0  # 追加
        }
    ...
    def generate(self, school: School, constraints: List[Constraint],
        ...
        """完璧な時間割を生成"""
        logger.info("=== Ultrathink Perfect Generator V8 開始 ===")
    ...
    def _initialize_teacher_availability(self, school: School, schedule: Schedule) -> Dict[str, Set[Tuple[str, int]]]:
        ...
        for time_slot, assignment in schedule.get_all_assignments():
            if assignment.teacher and assignment.teacher.name in availability:
                ...
                logger.debug(f"既存配置: {assignment.teacher.name}先生 @ {time_slot.day}{time_slot.period}限")
        
        # 統計情報
        total_busy = sum(30 - len(slots) for slots in availability.values())
        logger.info(f"教師利用可能性を初期化: {total_busy}個の既存配置を検出")
            ...
            for day in ["月", "火", "水", "木", "金"]:
                for period in range(1, 7):
                    ...
                    if parent_assignment and parent_assignment.subject.name in ["数", "英"]:
                        # 担任を探す
                        teacher = self._find_exchange_class_teacher(school, exchange_ref)
    ...
    # ヘルパーメソッド（V7と同じ実装だが、教師利用可能性の管理を強化）
    ...
    def _find_exchange_class_teacher(self, school: School, exchange_ref: ClassRef) -> Optional[Teacher]:
        """交流学級の担任を探す"""
        # 簡単のため、固定マッピング
        teacher_map = {
            (1, 6): "溝呂木", (1, 7): "智田",
        ...
        teacher_name = teacher_map.get((exchange_ref.grade, exchange_ref.class_number))
        if teacher_name:
            for teacher in school.get_all_teachers():
