#!/usr/bin/env python3
"""
Ultrathink Perfect Generator V4（修正版）
最初から完璧な時間割を生成するための革新的なジェネレーター

V3からの改善点：
1. 親学級に数学/英語を配置してから自立活動を配置
2. 6校時の固定処理を完全に実装
3. 固定科目（学、総、道など）を通常配置から除外
4. 配置順序の最適化
"""
...
class UltrathinkPerfectGeneratorV4:
    """最初から完璧な時間割を生成する革新的ジェネレーター（V4）"""
    ...
    def _place_fixed_and_test_periods(self, schedule: Schedule, school: School) -> None:
        """固定科目とテスト期間を配置（6校時も完全に処理）"""
        logger.info("【フェーズ1: 固定科目とテスト期間の配置】")
        placed = 0
        
        # 6校時の固定処理（CLAUDE.mdに従う）
        fixed_6th = {
            "月": "欠",     # Monday 6th: "欠" for all classes
            "火": "YT",     # Tuesday 6th: "YT"
            "水": "YT",     # Wednesday 6th: "YT"
            "木": "YT",     # Thursday 6th: "YT"
            "金": "欠"      # Friday 6th: "欠"
        }
        
        for day, subject in fixed_6th.items():
            for class_ref in school.get_all_classes():
                time_slot = TimeSlot(day=day, period=6)
                
                # 既に配置されている場合はスキップ（input.csvの内容を尊重）
                ...
                for period in range(1, 7):
                    ...
                    if exchange_assignment and exchange_assignment.subject.name == "自立":
                        ...
                        if not parent_assignment or parent_assignment.subject.name not in ["数", "英"]:
                            # 違反している自立活動を削除
                            logger.warning(f"違反: {exchange_ref} {time_slot} の自立活動 (親学級: {parent_assignment.subject.name if parent_assignment else '空き'})")
