#!/usr/bin/env python3
"""
Ultrathink Perfect Generator V11（教師情報完全復元版）
教師割り当て情報をCSVRepositoryとSchoolから正確に復元

V10からの改善点：
1. CSVRepositoryから教師情報を直接取得
2. 初期スケジュール読み込み時の教師情報を保持
3. teacher_subject_mapping.csvを直接参照
4. デバッグ情報を強化して問題箇所を特定
"""
...
class UltrathinkPerfectGeneratorV11:
    """最初から完璧な時間割を生成する革新的ジェネレーター（V11）"""
    ...
    def generate(self, school: School, constraints: List[Constraint],
        ...
        """完璧な時間割を生成"""
        logger.info("=== Ultrathink Perfect Generator V11 開始 ===")
        ...
        # 教師の利用可能時間を初期化（改善版）
        self.teacher_availability = self._initialize_teacher_availability_v11(school, schedule)
    ...
    def _initialize_teacher_availability_v11(self, school: School, schedule: Schedule) -> Dict[str, Set[Tuple[str, int]]]:
        """教師の利用可能時間を初期化（V11改善版）"""
        logger.info("=== 教師利用可能時間の初期化（V11） ===")
        
        # 全教師の初期化
        teacher_availability = {}
        for teacher in school.get_all_teachers():
            teacher_availability[teacher.name] = set()
        ...
        for time_slot, assignment in schedule.get_all_assignments():
            ...
            if teacher_name:
                if teacher_name not in teacher_availability:
                    teacher_availability[teacher_name] = set()
                teacher_availability[teacher_name].add((time_slot.day, time_slot.period))
                teacher_count += 1
        ...
        for teacher_name, busy_slots in sorted(teacher_availability.items()):
            if len(busy_slots) > 0:
