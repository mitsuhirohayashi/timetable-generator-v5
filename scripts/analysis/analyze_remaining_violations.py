...
def analyze_remaining_violations():
    """残った制約違反を詳細に分析"""
    ...
    for (day, period), teacher_classes in time_teacher_assignments.items():
        ...
        for teacher, assignments in teacher_classes.items():
            if len(assignments) > 1:
                ...
                # 実際の違反のみ
                if not is_grade5 and not (is_test_period and same_grade):
                    real_violations.append({
                        ...
                        'violation_type': '異学年同時担当' if not same_grade else '同学年複数クラス'
                    })
    ...
    print("\n【解決策の提案】")
    print("\n1. 教師重複違反への対処:")
    print("   - 月曜4・5校時: 最も違反が多い時間帯")
    print("   - 火曜5校時: 複数の教師が重複")
    print("   - 異学年同時担当は物理的に不可能なため、優先的に解決が必要")
    
    print("\n2. 体育館使用違反への対処:")
    print("   - 木曜3校時: 3クラスが同時使用（最も深刻）")
    print("   - 金曜2・3・5校時: 2クラスずつが重複")
    print("   - 体育の授業を他の時間帯に分散させる必要")
